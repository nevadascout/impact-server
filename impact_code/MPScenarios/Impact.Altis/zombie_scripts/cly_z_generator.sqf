			...
			if (!isNull (_zombies select _x)) then {
				...
				if ({side _x==east or side _x==west or side _x==resistance} count allGroups<432-CLY_hordereservedgroups) then {
					...
					if (_spawn) then {
						...
						[_zombie,_zombietype,objNull,_horde,0,_positions select _x,true] exec "impact_code\MPScenarios\Impact.Altis\zombie_scripts\cly_z_init.sqs";
...
_trig setTriggerActivation ["NONE", "PRESENT", FALSE];
	...
	_trig setTriggerStatements ["true", "0=[thisTrigger,-1] execVM 'impact_code\MPScenarios\Impact.Altis\zombie_scripts\cly_z_generator.sqf';", ""];
